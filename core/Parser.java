...
public class Parser {
	...
	private void parseLine(String line) throws UnrecognizedEntryException {
		...
		default -> throw new UnrecognizedEntryException(
				"Line with wong type: " + components[0]);
		}
	}

	private void checkComponentsLength(String[] components, int expectedSize,
			String line) throws UnrecognizedEntryException {
		if (components.length != expectedSize)
			throw new UnrecognizedEntryException(
					"Invalid number of fields in line: " + line);
	}

	// parse a client with format CLIENT|id|nome|taxId
	private void parseClient(String[] components, String line)
		...
		try {
			...
			throw new UnrecognizedEntryException(
					"Invalid number in line " + line, nfe);
		} catch (DuplicateEntityKeyException e) {
			throw new UnrecognizedEntryException(
					"Invalid specification in line: " + line, e);
		}
	}
	...
	private void parseTerminal(String[] components, String line)
		...
		try {
			Terminal terminal = _network.registerTerminal(components[1],
					components[2], components[0]);
			switch (components[3]) {
			case "SILENCE" -> terminal
					.changeTerminalMode(new TerminalModeSilence());
			case "OFF" -> terminal.changeTerminalMode(new TerminalModeOff());
			default -> {
				if (!components[3].equals("ON"))
					throw new UnrecognizedEntryException(
							"Invalid specification in line: " + line);
			}
			}
		} catch (UnknownKeyException | DuplicateEntityKeyException
				| KeyFormattingExemption e) {
			throw new UnrecognizedEntryException(
					"Invalid specification: " + line, e);
		}
	}

	// Parse a line with format FRIENDS|idTerminal|idTerminal1,...,idTerminalN
	private void parseFriends(String[] components, String line)
			throws UnrecognizedEntryException {
		...
		try {
			...
			throw new UnrecognizedEntryException(
					"Some message error in line:  " + line, e);
		}
	}
}